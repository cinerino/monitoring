...
function reportTimeLeftUntilEvent(sellerName, telemetries, measuredFrom, measuredThrough) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        params.chd += telemetries.map((telemetry) => Math.floor(telemetry.result.transactions.maxTimeLeftUntilEventInMilliseconds / HOUR_IN_MILLISECONDS))
            .join(',');
        // tslint:disable-next-line:prefer-template
        params.chd += '|' + telemetries.map((telemetry) => Math.floor(telemetry.result.transactions.averageTimeLeftUntilEventInMilliseconds / HOUR_IN_MILLISECONDS))
            .join(',');
        // tslint:disable-next-line:prefer-template
        params.chd += '|' + telemetries.map((telemetry) => Math.floor(telemetry.result.transactions.minTimeLeftUntilEventInMilliseconds / HOUR_IN_MILLISECONDS))
            .join(',');
        const imageFullsize = yield GoogleChart.publishUrl(params);
        debug('url published.', imageFullsize);
...
function reportTransactionAmounts(sellerName, telemetries, measuredFrom, measuredThrough) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        params.chd += telemetries.map((telemetry) => telemetry.result.transactions.averageAmount)
            .join(',');
        const imageFullsize = yield GoogleChart.publishUrl(params);
        debug('url published.', imageFullsize);
