...
export async function aggregateOfferAvailableHoursRateByScreen(theaterCode: string, screenBranchCode: string) {
    ...
    let events = await eventRepo.eventModel.find(
        ...
        .map((e) => {
            return {
                ...
                reserveStartDate: <Date>moment(`${e.coaInfo.rsvStartDate} 00:00:00+09:00`, 'YYYYMMDD HH:mm:ssZ').toDate(),
    ...
    const aggregations = seats.map((seat) => {
        ...
        const offeredHours = events.reduce(
            (a, b) => a + moment(b.startDate).diff(moment(<Date>b.firstOrderDate), TIME_UNIT),
            0
        );
        ...
        const availableHours = events.reduce(
            (a, b) => {
                ...
                if (order === undefined) {
                    return a + moment(b.startDate).diff(moment(<Date>b.firstOrderDate), TIME_UNIT);
                } else {
                    // 注文が入っていれば、最初の予約から自分の予約までの時間
                    return a + moment(order.orderDate).diff(moment(<Date>b.firstOrderDate), TIME_UNIT);
                }
            },
            0
        );
